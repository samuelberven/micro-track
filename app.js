...
app.delete('/delete-game-ajax/', function(req,res,next){
    ...
    let deleteGame = `DELETE FROM Games WHERE gameID = ?`;
          ...
          // Run the 1st query
          db.pool.query(deleteCustomersHaveGame, [gameID], function(error, rows, fields){
              if (error) {
...
app.put('/put-game-ajax', function(req,res,next){
    ...
    db.pool.query(query1, [developerID, title, description, releaseDate, gameID], function(error, rows, fields) {
        if (error) {
            console.log(error);
  ...
  {  
    ...
    db.pool.query(query1, function(error, rows, fields){
        ...
        db.pool.query(query2, (error, rows, fields) => {
            ...
            let games = rows;
            return res.render('microtransactions', {data: microtransactions, games: games});
        });
    });
...
app.delete('/delete-microtransaction-ajax/', function(req,res,next){
    ...
    let deleteMicrotransaction = `DELETE FROM Microtransactions WHERE microtransactionID = ?`;
            ...
            db.pool.query(deleteMicrotransaction, [microtransactionID], function(error, rows, fields) {
  ...
  db.pool.query(query1, function(error, rows, fields){
      ...
      db.pool.query(query2, (error, rows, fields) => {
          ...
          db.pool.query(query3, (error, rows, fields) => {
            ...
            let microtransactions = rows;
            return res.render('purchases', {data: purchases, customers: customers, microtransactions: microtransactions});
          });          
      });
  });
});
...
app.put('/put-purchase-ajax', function(req,res,next){
    ...
    if (customerID === 'NULL') {
        db.pool.query(query2, [microtransactionID, date, purchaseID], function(error, rows, fields) {
            if (error) {
                //If there is an error, log the error
                console.log(error);
                return res.sendStatus(400);
            } else {
                db.pool.query(purchasesQuery, [purchaseID], function(error, rows, fields) {
                    res.send({purchases: rows});
                });
            }
        });
    } else {
        db.pool.query(query1, [customerID, microtransactionID, date, purchaseID], function(error, rows, fields) {
            if (error) {
                //If there is an error, log the error
                console.log(error);
                return res.sendStatus(400);
...
{  
    // Declare Query 1
    let query1 = 
    `SELECT Customers.customerID, Customers.username, Customers.email, ServicePlatforms.platformName AS servicePlatform
    FROM Customers
    ...
    db.pool.query(query1, function(error, rows, fields){ // Execute the query
        ...
        db.pool.query(query2, (error, rows, fields) => {
            ...
            return res.render('customers', {data: customers, servicePlatforms: servicePlatforms});
        });
    });
});
    ...
    let deleteCustomersHaveGame = `DELETE FROM CustomersHaveGames WHERE customerID = ?`
            ...
            db.pool.query(deleteCustomer, [customerID], function(error, rows, fields) {
                ...
                if (error) {
                    ...
                    // Run the second query
                    db.pool.query(deleteCustomersHaveGame, [customerID], function(error, rows, fields) {
...
app.put('/put-customer-ajax', function(req,res,next){
    ...
    // Run the queries 1 through 3
    db.pool.query(query1, [servicePlatformID, username, email, customerID], function(error, rows, fields) {
        if (error) {
            //If there is an error, log the error
            console.log(error);
            return res.sendStatus(400);
        } else {
            db.pool.query(customersQuery, [customerID], function(error, rows, fields) {
...
app.delete('/delete-service-platform-ajax/', function(req,res,next){
    ...
    let deleteServicePlatform = `DELETE FROM ServicePlatforms WHERE servicePlatformID = ?`;
        ...
        // Run the query (Query 1)
        db.pool.query(deleteServicePlatform, [servicePlatformID], function(error, rows, fields) {
  ...
  db.pool.query(query1, function(error, rows, fields){
    ...
    db.pool.query(query2, (error, rows, fields) => {
        ...
        db.pool.query(query3, (error, rows, fields) => {
          ...
          // Save the games
          let games = rows;
          return res.render('customers-have-games', {data: customersHaveGames, customers: customers, games: games});
        });          
    });
});
});
...
app.put('/put-customers-have-games-ajax', function(req,res,next){
    ...
    db.pool.query(query1, [customerID, gameID, installDate, customersHaveGamesID], function(error, rows, fields) {
        if (error) {
            //If there is an error, log the error
            console.log(error);
            return res.sendStatus(400);
        } else {
            db.pool.query(query2, [customersHaveGamesID], function(error, rows, fields) {
...
app.delete('/delete-customers-have-games-ajax/', function(req,res,next){
    ...
    let deleteCustomersHaveGame = `DELETE FROM CustomersHaveGames WHERE customersHaveGamesID = ?`;
          ...
          // Run the 1st query
          db.pool.query(deleteCustomersHaveGame, [customersHaveGamesID], function(error, rows, fields){
...
app.put('/put-developer-ajax', function(req,res,next){
    ...
    // Run the query 1
    db.pool.query(query1, [developerName, address, city, state, zipCode, email, contact, developerID], function(error, rows, fields) {});
